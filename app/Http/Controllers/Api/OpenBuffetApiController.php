...
class OpenBuffetApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getOpenBuffetsCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('open_buffets','open_buffets.city_id','=','cities.id');
        $query->leftJoin('services','open_buffets.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('open_buffets.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','open_buffets.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('open_buffets.country_id', $country_id);
        } else {
    ...
    public function getAllOpenBuffets($category_id, $service_id, $country_id, $city_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = OpenBuffet::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('open_buffet_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationOpenBuffets($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = OpenBuffet::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedOpenBuffets($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = OpenBuffet::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleOpenBuffet($card_id,$lang): JsonResponse
    {
        app()->setLocale($lang);
        $query=OpenBuffet::select('open_buffets.*',
        ...
        $query->leftJoin('countries','open_buffets.country_id','=','countries.id');
        $query->leftJoin('cities','open_buffets.city_id','=','cities.id');

        $query->leftJoin('services','open_buffets.service_id','=','services.id');
        $query->leftJoin('categories','open_buffets.category_id','=','categories.id');
        if (is_numeric($card_id)) {
            $query->where('open_buffets.id', $card_id);
        } else {
            $query->whereRaw("(open_buffets.slug like '%" . $card_id . "%')");
        }
        $query->where('open_buffets.status',1);


        $open_buffets = $query->get();

        // return $barber_shops;
        $data=array();
        foreach($open_buffets as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
