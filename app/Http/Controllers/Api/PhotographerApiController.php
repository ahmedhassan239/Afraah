...
class PhotographerApiController extends Controller
{
    ...
    //
    public function getPhotographersCities($country_id,$service_id)
    {
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('photographers','photographers.city_id','=','cities.id');
        $query->leftJoin('services','photographers.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('photographers.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','photographers.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('photographers.country_id', $country_id);
        } else {
    ...
    public function getAllPhotographers($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = Photographer::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('photographer_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationPhotographers($country_id,$lang)
    {
        $query = Photographer::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedPhotographers($country_id,$lang)
    {
        $query = Photographer::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSinglePhotographer($photographer_id,$lang): \Illuminate\Http\JsonResponse
    {
        app()->setLocale($lang);
        $query=Photographer::select('photographers.*',
        ...
        $query->leftJoin('countries','photographers.country_id','=','countries.id');
        $query->leftJoin('cities','photographers.city_id','=','cities.id');

        $query->leftJoin('services','photographers.service_id','=','services.id');
        $query->leftJoin('categories','photographers.category_id','=','categories.id');
        if (is_numeric($photographer_id)) {
            $query->where('photographers.id', $photographer_id);
        } else {
            $query->whereRaw("(photographers.slug like '%" . $photographer_id . "%')");
        }
        $query->where('photographers.status',1);
