...
class BarberShopApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getBarberShopsCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('barber_shops','barber_shops.city_id','=','cities.id');
        $query->leftJoin('services','barber_shops.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('barber_shops.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','barber_shops.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('barber_shops.country_id', $country_id);
        } else {
    ...
    public function getAllBarberShops($category_id,$service_id,$country_id,$city_id): \Illuminate\Http\JsonResponse
    {
       
       $query = BarberShop::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('barber_shop_likes.user_id', '=', auth()->id());
       }]);

       $query = $query->paginate(12);
    
       $barber_shops = $query->map(function ($value) {
            return [
                'category' => [
    ...
    public function getDestinationBarberShops($country_id): \Illuminate\Http\JsonResponse
    {

        $query = BarberShop::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedBarberShops($country_id,$lang): \Illuminate\Http\JsonResponse
    {
        $query = BarberShop::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleBarberShop($barbershop_id,$lang): \Illuminate\Http\JsonResponse
    {
        app()->setLocale($lang);
        $query=BarberShop::select('barber_shops.*',
        ...
        $query->leftJoin('countries','barber_shops.country_id','=','countries.id');
        $query->leftJoin('cities','barber_shops.city_id','=','cities.id');

        $query->leftJoin('services','barber_shops.service_id','=','services.id');
        $query->leftJoin('categories','barber_shops.category_id','=','categories.id');
        if (is_numeric($barbershop_id)) {
            $query->where('barber_shops.id', $barbershop_id);
        } else {
            $query->whereRaw("(barber_shops.slug like '%" . $barbershop_id . "%')");
        }
        $query->where('barber_shops.status',1);


        $barber_shops = $query->get();


        $data=array();
        foreach($barber_shops as $item){
            $id= $item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
            ...
            $facebook_image=asset('storage/'.$item->facebook_image);
            $twitter_title=$item->twitter_title;
            $twitter_description=$item->twitter_description;
            $twitter_image=asset('storage/'.$item->twitter_image);
            ...
            if (is_array($item->package)) {
                ...
                foreach ($item->package as $key) {
                    $package[]=[
                        'id'=>$y,
                        'name'=>$key['attributes']['package_name'][$lang],
                        'image'=>asset('storage/'.$key['attributes']['package_image']),
                        'description'=>$key['attributes']['description'][$lang],
                        'price'=>$key['attributes']['package_price'],
