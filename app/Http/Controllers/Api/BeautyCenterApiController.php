...
class BeautyCenterApiController extends Controller
{
    ...
    public function getBeautyCentersCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('beauty_centers','beauty_centers.city_id','=','cities.id');
        $query->leftJoin('services','beauty_centers.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('beauty_centers.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','beauty_centers.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('beauty_centers.country_id', $country_id);
        } else {
    ...
    public function getAllBeautyCenters($category_id, $service_id, $country_id, $city_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = BeautyCenter::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('beauty_center_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationBeautyCenters($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = BeautyCenter::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedBeautyCenters($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = BeautyCenter::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleBeautyCenter($beautycenter_id, $lang): \Illuminate\Http\JsonResponse
    {
        app()->setLocale($lang);
        $query = BeautyCenter::select('beauty_centers.*',
        ...
        $query->leftJoin('countries', 'beauty_centers.country_id', '=', 'countries.id');
        $query->leftJoin('cities', 'beauty_centers.city_id', '=', 'cities.id');

        $query->leftJoin('services', 'beauty_centers.service_id', '=', 'services.id');
        $query->leftJoin('categories', 'beauty_centers.category_id', '=', 'categories.id');
        if (is_numeric($beautycenter_id)) {
            $query->where('beauty_centers.id', $beautycenter_id);
        } else {
            $query->whereRaw("(beauty_centers.slug like '%" . $beautycenter_id . "%')");
        }
        $query->where('beauty_centers.status', 1);


        $beauty_centers = $query->get();


        $data = array();
        foreach ($beauty_centers as $item) {
            $id = $item->id;
            $name = $item->name;
            $slug = $item->slug;
            $email = $item->email;
