...
class WeddingDressApiController extends Controller
{
    ...
    public function getWeddingDressesCities($country_id,$service_id)
    {
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('wedding_dresses','wedding_dresses.city_id','=','cities.id');
        $query->leftJoin('services','wedding_dresses.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('wedding_dresses.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','wedding_dresses.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('wedding_dresses.country_id', $country_id);
        } else {
    ...
    public function getWeddingDresses($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = WeddingDress::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('wedding_dress_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationWeddingDresses($country_id,$lang)
    {
        $query = WeddingDress::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedWeddingDresses($country_id,$lang)
    {
        $query = WeddingDress::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleWeddingDress($weddingdress_id,$lang)
    {
        app()->setLocale($lang);
        $query=WeddingDress::select('wedding_dresses.*',
        ...
        $query->leftJoin('countries','wedding_dresses.country_id','=','countries.id');
        $query->leftJoin('cities','wedding_dresses.city_id','=','cities.id');

        $query->leftJoin('services','wedding_dresses.service_id','=','services.id');
        $query->leftJoin('categories','wedding_dresses.category_id','=','categories.id');
        if (is_numeric($weddingdress_id)) {
            $query->where('wedding_dresses.id', $weddingdress_id);
        } else {
            $query->whereRaw("(wedding_dresses.slug like '%" . $weddingdress_id . "%')");
        }
        $query->where('wedding_dresses.status',1);


        $wedding_dresses = $query->get();

        // return $wedding_dresses;
        $data=array();
        foreach($wedding_dresses as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $code=$item->code;
            ...
            $atelier_name=$item->atelier_name;
            $atelier_location=$item->atelier_location;
            $atelier_number=$item->atelier_number;
            $atelier_email=$item->atelier_email;
            ...
            if(isset($data)){
                $data[]=[
                    ...
                    'atelier_location'=>$atelier_location,
                    'atelier_number'=>$atelier_number,
                    'atelier_email'=>$atelier_email,
                    'gallery'=>$gallery,
                    'banner' =>$banner,
                    'alt'=>$alt,
                    'thumb' =>$thumb,
