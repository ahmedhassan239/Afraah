...
class HoneyMoonApiController extends Controller
{

    public function __construct() {
        auth()->setDefaultDriver('api');
    }
    public function getHoneyMoonsCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('honey_moons','honey_moons.city_id','=','cities.id');
        $query->leftJoin('services','honey_moons.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('honey_moons.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','honey_moons.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('honey_moons.country_id', $country_id);
        } else {
    ...
    public function getAllHoneyMoons($category_id,$service_id,$country_id,$city_id): \Illuminate\Http\JsonResponse
    { 
       $query = HoneyMoon::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('honey_moon_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationHoneyMoons($country_id): \Illuminate\Http\JsonResponse
    {

        $query = HoneyMoon::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedHoneyMoons($country_id,$lang): \Illuminate\Http\JsonResponse
    {
        $query = HoneyMoon::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleHoneyMoon($honeymoon_id,$lang): JsonResponse
    {
        app()->setLocale($lang);
        $query=HoneyMoon::select('honey_moons.*',
        ...
        $query->leftJoin('countries','honey_moons.country_id','=','countries.id');
        $query->leftJoin('cities','honey_moons.city_id','=','cities.id');

        $query->leftJoin('services','honey_moons.service_id','=','services.id');
        $query->leftJoin('categories','honey_moons.category_id','=','categories.id');
        if (is_numeric($honeymoon_id)) {
            $query->where('honey_moons.id', $honeymoon_id);
        } else {
            $query->whereRaw("(honey_moons.slug like '%" . $honeymoon_id . "%')");
        }
        $query->where('honey_moons.status',1);


        $honey_moons = $query->get();
        $data=array();
        foreach($honey_moons as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
