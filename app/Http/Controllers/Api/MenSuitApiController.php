...
class MenSuitApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getMenSuitsCities($country_id, $service_id)
    {
        $query = City::select('cities.id', 'cities.name as city_name', 'cities.slug as city_slug');
        $query->leftJoin('men_suits', 'men_suits.city_id', '=', 'cities.id');
        $query->leftJoin('services', 'men_suits.service_id', '=', 'services.id');
        if (is_numeric($service_id)) {
            $query->where('men_suits.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries', 'men_suits.country_id', '=', 'countries.id');
        if (is_numeric($country_id)) {
            $query->where('men_suits.country_id', $country_id);
        } else {
    ...
    public function getMenSuits($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = MenSuit::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('men_suit_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationMenSuits($country_id,$lang)
    {
        $query = MenSuit::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedMenSuits($country_id,$lang)
    {
        $query = MenSuit::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleMenSuit($mensuit_id,$lang)
    {
        app()->setLocale($lang);
        $query=MenSuit::select('men_suits.*',
        ...
        $query->leftJoin('countries','men_suits.country_id','=','countries.id');
        $query->leftJoin('cities','men_suits.city_id','=','cities.id');

        $query->leftJoin('services','men_suits.service_id','=','services.id');
        $query->leftJoin('categories','men_suits.category_id','=','categories.id');
        if (is_numeric($mensuit_id)) {
            $query->where('men_suits.id', $mensuit_id);
        } else {
            $query->whereRaw("(men_suits.slug like '%" . $mensuit_id . "%')");
        }
        $query->where('men_suits.status',1);
        $men_suits = $query->get();
        $data=array();
        foreach($men_suits as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $code=$item->code;
