...
class WeddingPalaceApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }
    public function getWeddingPalacesCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('wedding_palaces','wedding_palaces.city_id','=','cities.id');
        $query->leftJoin('services','wedding_palaces.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('wedding_palaces.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','wedding_palaces.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('wedding_palaces.country_id', $country_id);
        } else {
    ...
    public function getWeddingPalaces($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = WeddingPalace::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('wedding_palace_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationWeddingPalaces($country_id,$lang)
    {
        $query = WeddingPalace::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedWeddingPalaces($country_id,$lang)
    {
        $query = WeddingPalace::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleWeddingPalace($wedding_palace_id,$lang)
    {
        app()->setLocale($lang);
        $query=WeddingPalace::select('wedding_palaces.*',
        ...
        $query->leftJoin('countries','wedding_palaces.country_id','=','countries.id');
        $query->leftJoin('cities','wedding_palaces.city_id','=','cities.id');

        $query->leftJoin('services','wedding_palaces.service_id','=','services.id');
        $query->leftJoin('categories','wedding_palaces.category_id','=','categories.id');
        if (is_numeric($wedding_palace_id)) {
            $query->where('wedding_palaces.id', $wedding_palace_id);
        } else {
            $query->whereRaw("(wedding_palaces.slug like '%" . $wedding_palace_id . "%')");
        }
        $query->where('wedding_palaces.status',1);
        $wedding_palaces = $query->get();

        $data=array();
        foreach($wedding_palaces as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
            ...
            if (is_array($item->hall)) {
                ...
                foreach ($item->hall as $key) {
                    ...
                    $hall[]=[
                        'id'=>$y,
                        'key'=>$key['key'],
                        'name'=>$key['attributes']['hall_name']['ar'],
                        'slug'=>$key['attributes']['hall_slug'],
                        'thumb'=>asset('storage/'.$key['attributes']['hall_thumb']),
                        'thumb_alt'=>$key['attributes']['hall_thumb_alt'][$lang],
                        'capacity'=>$key['attributes']['hall_capacity'],
                        'price'=>$key['attributes']['hall_price'],
                    ];
                    $y++;
                }
            }
