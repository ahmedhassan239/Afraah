...
class GardenClubApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getGardenClubsCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('gardens_clubs','gardens_clubs.city_id','=','cities.id');
        $query->leftJoin('services','gardens_clubs.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('gardens_clubs.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','gardens_clubs.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('gardens_clubs.country_id', $country_id);
        } else {
    ...
    public function getAllGardenClubs($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = GardenAndClub::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('garden_clubs_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationGardenClubs($country_id,$lang)
    {
        $query = GardenAndClub::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedGardenClubs($country_id,$lang)
    {
        $query = GardenAndClub::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleGardenClub($gardenclub_id,$lang)
    {
        app()->setLocale($lang);
        $query=GardenAndClub::select('gardens_clubs.*',
        ...
        $query->leftJoin('countries','gardens_clubs.country_id','=','countries.id');
        $query->leftJoin('cities','gardens_clubs.city_id','=','cities.id');

        $query->leftJoin('services','gardens_clubs.service_id','=','services.id');
        $query->leftJoin('categories','gardens_clubs.category_id','=','categories.id');
        if (is_numeric($gardenclub_id)) {
            $query->where('gardens_clubs.id', $gardenclub_id);
        } else {
            $query->whereRaw("(gardens_clubs.slug like '%" . $gardenclub_id . "%')");
        }
        $query->where('gardens_clubs.status',1);


        $garden_clubs = $query->get();

        // return $barber_shops;
        $data=array();
        foreach($garden_clubs as $item){
            $garden_club_id=$item->id;
            $garden_club_name=$item->name;
            $garden_club_slug=$item->slug;
            $garden_club_email=$item->email;
            $garden_club_phone=$item->phone;
            $garden_club_description=$item->description;
            ...
            if(isset($data)){
                $data[]=[
                   ...
                   'category'=>[
                    ...
                    'id'=>$garden_club_id,
                    'name'=>$garden_club_name,
                    'slug'=>$garden_club_slug,
                    'phone'=>$garden_club_phone,
                    'email'=>$garden_club_email,
                    'description'=>$garden_club_description,
                    'user_id'=>$user_id,
                    'gallery'=>$gallery,
                    'banner' =>$banner,
                    'alt'=>$alt,
