...
class WeddingFlowerApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getWeddingFlowersCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('wedding_flowers','wedding_flowers.city_id','=','cities.id');
        $query->leftJoin('services','wedding_flowers.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('wedding_flowers.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','wedding_flowers.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('wedding_flowers.country_id', $country_id);
        } else {
    ...
    public function getAllWeddingFlowers($category_id, $service_id, $country_id, $city_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingFlower::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('wedding_flower_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationWeddingFlowers($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingFlower::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedWeddingFlowers($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingFlower::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleWeddingFlower($wedding_flower_id,$lang): JsonResponse
    {
        app()->setLocale($lang);
        $query= WeddingFlower::select('wedding_flowers.*',
            ...
            'wedding_flower_likes.like_type as wedding_flower_like_type'

        );
        $query->leftJoin('countries','wedding_flowers.country_id','=','countries.id');
        $query->leftJoin('cities','wedding_flowers.city_id','=','cities.id');
        $query->leftJoin('wedding_flower_likes', function ($join){
            $join->on( 'wedding_flowers.id', '=', 'wedding_flower_likes.article_id');
            $join->where( 'wedding_flower_likes.user_id', '=', auth()->id());
        });
        $query->leftJoin('services','wedding_flowers.service_id','=','services.id');
        $query->leftJoin('categories','wedding_flowers.category_id','=','categories.id');
        if (is_numeric($wedding_flower_id)) {
            $query->where('wedding_flowers.id', $wedding_flower_id);
        } else {
            $query->whereRaw("(wedding_flowers.slug like '%" . $wedding_flower_id . "%')");
        }
        $query->where('wedding_flowers.status',1);


        $wedding_flowers = $query->get();
        $data=array();
        foreach($wedding_flowers as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
            ...
            $like_type= ($item->wedding_flower_like_type)?$item->wedding_flower_like_type:0;
