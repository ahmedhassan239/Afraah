...
class MakeupArtistApiController extends Controller
{
    public function __construct()
    {
        auth()->setDefaultDriver('api');
    }
    public function getMakeupArtistsCities($country_id, $service_id)
    {
        $query = City::select('cities.id', 'cities.name as city_name', 'cities.slug as city_slug');
        $query->leftJoin('makeup_artists', 'makeup_artists.city_id', '=', 'cities.id');
        $query->leftJoin('services', 'makeup_artists.service_id', '=', 'services.id');
        if (is_numeric($service_id)) {
            $query->where('makeup_artists.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }
        

        $query->leftJoin('countries', 'makeup_artists.country_id', '=', 'countries.id');
        if (is_numeric($country_id)) {
            $query->where('makeup_artists.country_id', $country_id);
        } else {
    ...
    public function getAllMakeupArtists($category_id, $service_id, $country_id, $city_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = MakeupArtist::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('makeup_artist_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationMakeupArtists($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = MakeupArtist::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedMakeupArtists($country_id,$lang): \Illuminate\Http\JsonResponse
    {
        $query = MakeupArtist::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleMakeupArtist($MakeupArtist_id, $lang): \Illuminate\Http\JsonResponse
    {
        app()->setLocale($lang);
        $query = MakeupArtist::select(
            'makeup_artists.*',
        ...
        $query->leftJoin('countries', 'makeup_artists.country_id', '=', 'countries.id');
        $query->leftJoin('cities', 'makeup_artists.city_id', '=', 'cities.id');

        $query->leftJoin('services', 'makeup_artists.service_id', '=', 'services.id');
        $query->leftJoin('categories', 'makeup_artists.category_id', '=', 'categories.id');
        if (is_numeric($MakeupArtist_id)) {
            $query->where('makeup_artists.id', $MakeupArtist_id);
        } else {
            $query->whereRaw("(makeup_artists.slug like '%" . $MakeupArtist_id . "%')");
        }
        $query->where('makeup_artists.status', 1);


        $makeup_artists = $query->get();


        $data = array();
        foreach ($makeup_artists as $item) {
            $id = $item->id;
            $name = $item->name;
            $slug = $item->slug;
            $email = $item->email;
