...
class HairDresserApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }
    //
    public function getHairDressersCities($country_id, $service_id)
    {
        $query = City::select('cities.id', 'cities.name as city_name', 'cities.slug as city_slug');
        $query->leftJoin('hairdressers', 'hairdressers.city_id', '=', 'cities.id');
        $query->leftJoin('services', 'hairdressers.service_id', '=', 'services.id');
        if (is_numeric($service_id)) {
            $query->where('hairdressers.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries', 'hairdressers.country_id', '=', 'countries.id');
        if (is_numeric($country_id)) {
            $query->where('hairdressers.country_id', $country_id);
        } else {
    ...
    public function getAllHairDressers($category_id,$service_id,$country_id,$city_id,$lang): JsonResponse
    {
        $query = HairDresser::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('hairdresser_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationHairDressers($country_id,$lang):JsonResponse
    {
        $query = HairDresser::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedHairDressers($country_id,$lang): JsonResponse
    {
        $query = HairDresser::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleHairDresser($hairdresser_id,$lang): JsonResponse
    {
        app()->setLocale($lang);
        $query=HairDresser::select('hairdressers.*',
        ...
        $query->leftJoin('countries','hairdressers.country_id','=','countries.id');
        $query->leftJoin('cities','hairdressers.city_id','=','cities.id');

        $query->leftJoin('services','hairdressers.service_id','=','services.id');
        $query->leftJoin('categories','hairdressers.category_id','=','categories.id');
        if (is_numeric($hairdresser_id)) {
            $query->where('hairdressers.id', $hairdresser_id);
        } else {
            $query->whereRaw("(hairdressers.slug like '%" . $hairdresser_id . "%')");
                        }
        $query->where('hairdressers.status',1);
