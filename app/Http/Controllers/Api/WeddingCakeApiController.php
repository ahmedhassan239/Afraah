...
class WeddingCakeApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }

    public function getWeddingCakesCities($country_id,$service_id){
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('wedding_cakes','wedding_cakes.city_id','=','cities.id');
        $query->leftJoin('services','wedding_cakes.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('wedding_cakes.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','wedding_cakes.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('wedding_cakes.country_id', $country_id);
        } else {
    ...
    public function getAllWeddingCakes($category_id, $service_id, $country_id, $city_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingCake::where('status',1)->with('category')
       ...
       $query->with(['likes'=>function($query){
        $query->where('wedding_cake_likes.user_id', '=', auth()->id());
       }]);
    ...
    public function getDestinationWeddingCakes($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingCake::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedWeddingCakes($country_id, $lang): \Illuminate\Http\JsonResponse
    {
        $query = WeddingCake::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleWeddingCake($card_id,$lang): JsonResponse
    {
        app()->setLocale($lang);
        $query=WeddingCake::select('wedding_cakes.*',
        ...
        $query->leftJoin('countries','wedding_cakes.country_id','=','countries.id');
        $query->leftJoin('cities','wedding_cakes.city_id','=','cities.id');

        $query->leftJoin('services','wedding_cakes.service_id','=','services.id');
        $query->leftJoin('categories','wedding_cakes.category_id','=','categories.id');
        if (is_numeric($card_id)) {
            $query->where('wedding_cakes.id', $card_id);
        } else {
            $query->whereRaw("(wedding_cakes.slug like '%" . $card_id . "%')");
        }
        $query->where('wedding_cakes.status',1);


        $wedding_cakes = $query->get();

        // return $barber_shops;
        $data=array();
        foreach($wedding_cakes as $item){
            $id=$item->id;
            $name=$item->name;
            $slug=$item->slug;
            $email=$item->email;
