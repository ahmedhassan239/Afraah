...
class DjApiController extends Controller
{
    public function __construct() {
        auth()->setDefaultDriver('api');
    }
    public function getDjsCities($country_id,$service_id)
    {
        $query = City::select('cities.id','cities.name as city_name','cities.slug as city_slug');
        $query->leftJoin('djs','djs.city_id','=','cities.id');
        $query->leftJoin('services','djs.service_id','=','services.id');
        if (is_numeric($service_id)) {
            $query->where('djs.service_id', $service_id);
        } else {
            $query->whereRaw("(services.slug like '%" . $service_id . "%')");
        }

        $query->leftJoin('countries','djs.country_id','=','countries.id');
        if (is_numeric($country_id)) {
            $query->where('djs.country_id', $country_id);
        } else {
    ...
    public function getDjs($category_id,$service_id,$country_id,$city_id,$lang)
    {
        $query = Dj::where('status', 1)->with('category')
        ...
        $query->with(['likes' => function ($query) {
            $query->where('dj_likes.user_id', '=', auth()->id());
        }]);
    ...
    public function getDestinationDjs($country_id,$lang)
    {
        $query = Dj::where('status',1)->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getFeaturedDjs($country_id,$lang)
    {
        $query = Dj::where('status',1)->where('feature','1')->with('category','service','city');
        $query->with('country')->whereHas('country',function ($x) use($country_id){
            return $x->where('id',$country_id)->orWhere('slug',$country_id)->where('status',1);
        });
        $query->with(['likes'=>function($query){
    ...
    public function getSingleDj($dj_id,$lang): \Illuminate\Http\JsonResponse
    {
        app()->setLocale($lang);
        $query=Dj::select('djs.*',
        ...
        $query->leftJoin('countries','djs.country_id','=','countries.id');
        $query->leftJoin('cities','djs.city_id','=','cities.id');

        $query->leftJoin('services','djs.service_id','=','services.id');
        $query->leftJoin('categories','djs.category_id','=','categories.id');
        if (is_numeric($dj_id)) {
            $query->where('djs.id', $dj_id);
        } else {
            $query->whereRaw("(djs.slug like '%" . $dj_id . "%')");
        }
        $query->where('djs.status',1);
        $djs = $query->get();
        $data=array();
        foreach($djs as $item){
            $id=$item->id;
